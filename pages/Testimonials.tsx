    ...
    <div className="max-w-5xl mx-auto px-6 py-12">
      <div className="text-center mb-16">
        ...
        <button
          onClick={() => {
            if (!user) { onAuthClick(); return; }
            setEditingPost(null); setNewTitle(''); setNewContent('');
            setMediaItems([]); setNewRating(5); setVideoLink('');
            setShowAdd(true);
          }}
          ...
          <div key={t.id} className="break-inside-avoid bg-white p-10 rounded-[2.5rem] border border-slate-100 shadow-sm group hover:shadow-xl transition-all relative overflow-hidden">
            {user && (user.uid === t.authorId || user.role === 'admin') && (
              <div className="absolute top-4 right-4 flex gap-2">
                <button onClick={() => { setEditingPost(t); setNewTitle(t.title || ''); setNewContent(t.content); setNewRating(t.rating); if (t.media?.length) setMediaItems(t.media); setShowAdd(true); }} className="p-2 text-slate-400 hover:text-blue-600 bg-white/80 rounded-full"><Edit2 size={16} /></button>
            ...
            <p className="text-slate-500 italic leading-relaxed mb-8 whitespace-pre-wrap">{t.content}</p>

            {t.media && t.media.length > 0 && (
              <div className={`mb-6 grid gap-2 ${t.media.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>
                {t.media.map((m, i) => (
