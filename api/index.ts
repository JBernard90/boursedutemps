...
console.log('[Server] Démarrage en mode ESM...');

// Init DB en arrière-plan (non-bloquant pour Vercel)
initDB().catch((err) =>
  console.error('[DB] Erreur init (non bloquante):', err?.message || err)
);
...
const sendSMS = async (to: string, message: string): Promise<boolean> => {
  ...
  if (!apiKey || !deviceId) {
    console.warn('[TextBee] API Key ou Device ID manquant — SMS ignoré');
    return false;
  }
  ...
  try {
    ...
    console.log('[TextBee] Réponse:', data);
    return response.ok;
  } catch (error: any) {
...
// Force Content-Type JSON sur toutes les réponses /api
app.use('/api', (_req, res, next) => {
  res.setHeader('Content-Type', 'application/json; charset=utf-8');
  next();
});

// Parser JSON — intercepte les corps invalides
app.use(
  express.json({
    limit: '10mb',
    strict: false,
...
// Erreur de parsing JSON → JSON (pas HTML)
app.use(
  (
    err: any,
    ...
    if (err.type === 'entity.parse.failed') {
      return res
        .status(400)
        .json({ error: 'Corps JSON invalide', success: false });
    }
    next(err);
  }
);
...
app.post('/api/verify/init', async (req, res) => {
  ...
  if (!phone || !/^\+[1-9]\d{1,14}$/.test(phone)) {
    return sendError(
      res,
      'Format de téléphone invalide. Utilisez le format international (ex: +221...)',
      400,
      'INVALID_PHONE'
    );
  }
  ...
  try {
    await query(
      'DELETE FROM otps WHERE identifier = $1 OR identifier = $2',
      [`email:${email}`, `phone:${phone}`]
    );
    await query(
      'INSERT INTO otps (identifier, code, expires_at) VALUES ($1, $2, $3)',
      [`email:${email}`, emailOtp, expiresAt]
    );
    await query(
      'INSERT INTO otps (identifier, code, expires_at) VALUES ($1, $2, $3)',
      [`phone:${phone}`, phoneOtp, expiresAt]
    );
    ...
    transporter
      ...
      .catch((emailError: any) => {
        console.error('[Verify Init] Email non envoyé:', emailError.message);
        if (emailError.message?.includes('Application-specific password')) {
          console.error(
            '>>> Utilisez un mot de passe d\'application Google : https://myaccount.google.com/apppasswords'
          );
        }
    ...
    sendSMS(
      phone,
      `Bourse du Temps: Votre code de sécurité est ${phoneOtp}. Valable 10 min.`
    )
      .then((sent) => {
        if (!sent) console.log(`[DEV] Phone OTP pour ${phone}: ${phoneOtp}`);
      })
      .catch(() =>
        console.log(`[DEV] Phone OTP pour ${phone}: ${phoneOtp}`)
      );

    res.json({
      success: true,
      message: 'Codes envoyés. Vérifiez votre email et SMS.',
    });
  } catch (error: any) {
...
app.post('/api/verify/check', async (req, res) => {
  ...
  try {
    const emailResult = await query(
      'SELECT * FROM otps WHERE identifier = $1 ORDER BY created_at DESC LIMIT 1',
      [`email:${email}`]
    );
    ...
    if (
      !storedEmail ||
      storedEmail.code !== emailCode ||
      new Date() > new Date(storedEmail.expires_at)
    ) {
      return sendError(res, 'Code email invalide ou expiré.', 400, 'INVALID_EMAIL_CODE');
    }
    if (
      !storedPhone ||
      storedPhone.code !== phoneCode ||
      new Date() > new Date(storedPhone.expires_at)
    ) {
...
app.post('/api/register', async (req, res) => {
  ...
  try {
    ...
    if (
      ...
      return sendError(
        res,
        'Échec de sécurité : Code email invalide ou expiré.',
        403,
        'INVALID_EMAIL_CODE'
      );
    }
    if (
      ...
      return sendError(
        res,
        'Échec de sécurité : Code SMS invalide ou expiré.',
        403,
        'INVALID_SMS_CODE'
      );
    }
    ...
    await query(
      `INSERT INTO users
        (uid, email, password, first_name, last_name, whatsapp, campus, department,
         gender, country, offered_skills, requested_skills, availability, languages,
         avatar, verified, is_verified_email, is_verified_sms)
    ...
    // Erreur doublon PostgreSQL (race condition)
    if (error.code === '23505') {
      return sendError(res, 'Cet email est déjà utilisé.', 409, 'EMAIL_EXISTS');
    }
  ...
  app.get(`/api/${table}/:id`, async (req, res) => {
    try {
      ...
      const result = await query(
        `SELECT * FROM ${dbTable} WHERE ${idCol} = $1`,
        [req.params.id]
      );
      if (result.rows.length === 0)
        return sendError(res, 'Ressource introuvable', 404, 'NOT_FOUND');
  ...
  app.put(`/api/${table}/:id`, authenticateToken, async (req: AuthRequest, res) => {
    try {
      ...
      const existing = await query(
        `SELECT ${idCol} FROM ${dbTable} WHERE ${idCol} = $1`,
        [req.params.id]
      );
...
app.use(
  (
    ...
    if (err.code === '23505') {
      return res
        .status(409)
        .json({ error: 'Valeur déjà existante (doublon)', success: false, code: 'DUPLICATE' });
    }
